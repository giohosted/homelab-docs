...
*Update this file whenever hardware is added, swapped, or retired.*  
...
| **Boot Drive 1** | Samsung NVMe 1TB *(fill in exact model + serial when installed)* |

| **Boot Drive 2** | Sabrent NVMe 1TB *(fill in exact model + serial when installed)* |
...
| **OS** | Proxmox VE *(fill in version when installed)* |
...
| **Boot Storage** | *(fill in — NVMe model + capacity)* |
...
| **OS** | *(fill in — Raspberry Pi OS / Ubuntu)* |

| **IP** | 192.168.10.20 (VLAN 10 — Management) |

| **Source** | *(fill in — existing? purchased when?)* |
...
| **Source** | *(fill in — Amazon / UI store?)* |
...
*See `architecture/storage.md` for full pool layout, assignment, and rationale. Summary below.*
